...
interface MCPRoleCardProps {
  role: MCPRole;
  mcpPermissions: MCPermission[];
  onEdit: (role: MCPRole) => void;
  onDelete: (roleId: string) => void;
  onTogglePermission: (roleId: string, permissionId: string) => void;
}

export const MCPRoleCard = ({ role, mcpPermissions, onEdit, onDelete, onTogglePermission }: MCPRoleCardProps) => {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent>
        <div className="space-y-2">
          <Label className="text-sm font-medium">MCP权限</Label>
              ...
              return (
                ...
                >
                  <div className="flex items-start space-x-2">
                    <Checkbox
                      id={`mcp-perm-${role.id}-${permission.id}`}
                    ...
                    <div className="flex-1">
                      <Label
                        htmlFor={`mcp-perm-${role.id}-${permission.id}`}
